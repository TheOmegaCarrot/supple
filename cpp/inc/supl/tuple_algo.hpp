...
constexpr void for_each_in_tuple(const Tuple& tup, Func&& func) noexcept(
    noexcept(impl::for_each_in_tuple_impl(
...
constexpr void for_each_in_subtuple_impl(
    const Tuple& tup, Func&& func, std::index_sequence<Idxs...>,
    index_constant<
...
template <typename Tuple, typename Func, std::size_t... Inds>
constexpr auto
tuple_transform_impl(const Tuple& tup, Func&& func, std::index_sequence<Inds...>) noexcept(
    // calls are noexcept
...
tuple_transform(const Tuple& tup, Func&& func) noexcept(
    noexcept(impl::tuple_transform_impl(
        tup, std::forward<Func>(func),
        std::make_index_sequence<std::tuple_size_v<Tuple>> {})))
{
  constexpr auto seq {
...
[[nodiscard]] constexpr auto tuple_any_of_impl(
    const Tuple& tup, Pred&& pred,
    std::index_sequence<
...
[[nodiscard]] constexpr auto
tuple_any_of(const Tuple& tup, Pred&& pred) noexcept(
    noexcept(impl::tuple_any_of_impl(
        tup, std::forward<Pred>(pred),
        std::make_index_sequence<std::tuple_size_v<Tuple>> {}))) -> bool
{
  constexpr auto seq {
      std::make_index_sequence<std::tuple_size_v<Tuple>> {}};
...
namespace impl {
template <typename Tuple, typename Pred, std::size_t... Inds>
[[nodiscard]] constexpr auto tuple_all_of_impl(
    const Tuple& tup, Pred&& pred,
    std::index_sequence<
        Inds...>) noexcept(noexcept((supl::invoke(pred,
                                                  std::get<Inds>(tup))
...
template <typename Tuple, typename Pred>
[[nodiscard]] constexpr auto
tuple_all_of(const Tuple& tup, Pred&& pred) noexcept(
    noexcept(impl::tuple_all_of_impl(
        tup, std::forward<Pred>(pred),
        std::make_index_sequence<std::tuple_size_v<Tuple>> {}))) -> bool
{
  constexpr auto seq {
...
template <typename Tuple, typename Pred>
[[nodiscard]] constexpr auto
tuple_none_of(const Tuple& tup, Pred&& pred) noexcept(
    noexcept(tuple_any_of(tup, std::forward<Pred>(pred)))) -> bool
{
...
[[nodiscard]] constexpr auto
tuple_push_back_impl(const Tuple& tup, std::index_sequence<Inds...>, Ts&&... data) noexcept(
    tl::all_of_v<
        Tuple,
        std::
            is_nothrow_copy_constructible> && (std::is_nothrow_copy_constructible_v<Ts> && ...))
...
[[nodiscard]] constexpr auto
tuple_push_back(const Tuple& tup, Ts&&... data) noexcept(
    tl::all_of_v<
        Tuple,
        std::
            is_nothrow_copy_constructible> && (std::is_nothrow_copy_constructible_v<Ts> && ...))
...
[[nodiscard]] constexpr auto tuple_pop_back_impl(
    const Tuple& tup,
    std::index_sequence<
        Inds...>) noexcept(tl::all_of_v<Tuple,
                                        std::
...
[[nodiscard]] constexpr auto tuple_pop_back(const Tuple& tup) noexcept(
    tl::all_of_v<tl::pop_back_t<Tuple>,
                 std::is_nothrow_copy_constructible>)
    -> tl::pop_back_t<Tuple>
{
  constexpr auto seq {
...
[[nodiscard]] constexpr auto
tuple_push_front_impl(const Tuple<Pack...>& tup, std::index_sequence<Inds...>, Ts&&... data) noexcept(
    tl::all_of_v<
        Tuple<Pack...>,
        std::
            is_nothrow_copy_constructible> && (std::is_nothrow_copy_constructible_v<Ts> && ...))
...
template <typename Tuple, typename... Ts>
[[nodiscard]] constexpr auto
tuple_push_front(const Tuple& tup, Ts&&... data) noexcept(
    tl::all_of_v<
        Tuple,
        std::
            is_nothrow_copy_constructible> && (std::is_nothrow_copy_constructible_v<Ts> && ...))
...
template <typename As, typename Param, typename Tuple, std::size_t... Idxs>
[[nodiscard]] constexpr auto tuple_push_front_as_impl(
    const Tuple& tup, std::index_sequence<Idxs...>,
    Param&&
        data) noexcept(tl::all_of_v<Tuple,
                                    std::is_nothrow_copy_constructible>&&
...
template <typename As, typename Param, typename Tuple>
[[nodiscard]] constexpr auto
tuple_push_front_as(const Tuple& tup, Param&& data) noexcept(
    tl::all_of_v<Tuple, std::is_nothrow_copy_constructible>&&
        std::is_nothrow_constructible_v<As, Param>)
    -> tl::push_front_t<Tuple, As>
{
  constexpr auto seq {
      std::make_index_sequence<std::tuple_size_v<Tuple>> {}};
  return impl::tuple_push_front_as_impl<As>(tup, seq,
                                            std::forward<Param>(data));
}

namespace impl {
template <typename Tuple, std::size_t... Inds>
[[nodiscard]] constexpr auto tuple_pop_front_impl(
    const Tuple& tup,
    std::index_sequence<
        Inds...>) noexcept(tl::all_of_v<tl::pop_front_t<Tuple>,
                                        std::
                                            is_nothrow_copy_constructible>)
...
[[nodiscard]] constexpr auto tuple_rotate_left_impl(
    const Tuple& tup,
    std::index_sequence<
...
namespace impl {
template <typename Tuple, std::size_t... Idxs>
[[nodiscard]] constexpr auto tuple_rotate_right_impl(
    const Tuple& tup,
    std::index_sequence<
        Idxs...>) noexcept(tl::all_of_v<Tuple,
                                        std::
...
[[nodiscard]] constexpr auto tuple_insert_impl(
    ...
    Ts&&... data) noexcept(tl::
                               all_of_v<
                                   Tuple,
                                   std::
                                       is_nothrow_copy_constructible> && (std::is_nothrow_constructible_v<Ts, Ts> && ...))
...
[[nodiscard]] constexpr auto
tuple_insert(const Tuple& tup, Ts&&... data) noexcept(
    tl::all_of_v<
        Tuple,
        std::
            is_nothrow_copy_constructible> && (std::is_nothrow_constructible_v<Ts, Ts> && ...))
...
template <typename Tuple, std::size_t... pre_idxs,
          std::size_t... post_idxs>
[[nodiscard]] constexpr auto tuple_erase_impl(
    const Tuple& tup, std::index_sequence<pre_idxs...>,
    std::index_sequence<
...
[[nodiscard]] constexpr auto tuple_replace_impl(
    const Tuple& tup, T&& data, std::index_sequence<Pre_Idxs...>,
    std::index_sequence<Post_Idxs...>,
    index_constant<
        Idx>) noexcept(tl::all_of_v<tl::replace_t<Tuple, Idx,
                                                  remove_cvref_t<T>>,
                                    std::is_nothrow_copy_constructible>)
...
[[nodiscard]] constexpr auto
tuple_replace(const Tuple& tup, T&& data) noexcept(
...
template <typename Tuple, std::size_t... Pre_Idxs,
          std::size_t... Post_Idxs, std::size_t Idx>
[[nodiscard]] constexpr auto tuple_split_impl(
    const Tuple& tup, std::index_sequence<Pre_Idxs...>,
    std::index_sequence<Post_Idxs...>,
    index_constant<
...
[[nodiscard]] constexpr auto subtuple_impl(
    const Tuple& tup, std::index_sequence<Inds...>, index_constant<Begin>,
    index_constant<End>) noexcept((std::
                                       is_nothrow_copy_constructible_v<
                                           decltype(std::get<Inds + Begin>(
                                               tup))> && ...))
...
/* }}} */
template <std::size_t Begin, std::size_t End, typename Tuple>
[[nodiscard]] constexpr auto subtuple(const Tuple& tup) noexcept(noexcept(
    impl::subtuple_impl(tup, std::make_index_sequence<End - Begin> {},
...
namespace impl {
template <typename Tuple, typename Pred, std::size_t... Inds>
constexpr auto tuple_count_if_impl(
    const Tuple& tup, Pred&& pred,
    std::index_sequence<
        Inds...>) noexcept(noexcept((static_cast<std::
                                                     size_t>(supl::invoke(
                                         pred, std::get<Inds>(tup)))
                                     + ...)
...
template <typename Tuple, typename Pred>
[[nodiscard]] constexpr auto
tuple_count_if(const Tuple& tup, Pred&& pred) noexcept(
    noexcept(impl::tuple_count_if_impl(
        tup, std::forward<Pred>(pred),
        std::make_index_sequence<std::tuple_size_v<Tuple>> {})))
    -> std::size_t
{
...
constexpr auto tuple_interleave_impl(
...
[[nodiscard]] constexpr auto
tuple_interleave(const Tuple1& tup1, const Tuple2& tup2) noexcept(
...
namespace impl {
template <typename Tuple, std::size_t... Idxs>
[[nodiscard]] constexpr auto tuple_front_n_impl(
    const Tuple& tup,
    std::index_sequence<
        Idxs...>) noexcept(tl::
                               all_of_v<
...
[[nodiscard]] constexpr auto tuple_back_n_impl(
    const Tuple& tup, std::index_sequence<Idxs...>,
    supl::index_constant<
        Offset>) noexcept(tl::all_of_v<tl::back_n_t<Tuple,
                                                    sizeof...(Idxs)>,
                                       std::is_nothrow_copy_constructible>)
...
template <template <typename> typename Transform, typename Tuple,
          std::size_t... Idxs>
[[nodiscard]] constexpr auto tuple_type_transform_impl(
    const Tuple& tup,
    std::index_sequence<
