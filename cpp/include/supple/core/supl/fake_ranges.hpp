...
auto all_of(Container&& container, Pred&& pred)
  noexcept(noexcept(std::all_of(
...
template <typename Container, typename Pred>
auto any_of(Container&& container, Pred&& pred)
  noexcept(noexcept(std::any_of(
    std::begin(container), std::end(container), std::forward<Pred>(pred)
  ))) -> bool
{
    return std::any_of(
...
template <typename Container, typename Pred>
auto none_of(Container&& container, Pred&& pred)
  noexcept(noexcept(std::none_of(
    std::begin(container), std::end(container), std::forward<Pred>(pred)
  ))) -> bool
{
    return std::none_of(
...
auto for_each(Container&& container, Func&& func)
  noexcept(noexcept(std::for_each(
...
auto for_each_n(Container&& container, Size n, Func&& func)
  noexcept(noexcept(std::for_each_n(
    std::begin(container), std::end(container), n, std::forward<Func>(func)
  ))) -> Func
{
    return std::for_each_n(
      std::begin(container), std::end(container), n,
...
) -> typename std::iterator_traits<decltype(std::begin(container)
)>::difference_type
{
...
private:
    ...
    struct iterator_base
    {
        ...
        [[nodiscard]] friend constexpr auto operator==(
          const iterator_base& lhs, const iterator_base& rhs
        ) noexcept -> bool
        {
