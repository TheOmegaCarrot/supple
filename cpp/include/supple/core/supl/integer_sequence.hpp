...
#define integer_sequence_binary_operation(OP)                                                                  \
    ...
    constexpr auto                                                                                             \
    operator OP(::supl::integer_sequence<lhs, lhs_seq...>, ::supl::integer_sequence<rhs, rhs_seq...>) noexcept \
      -> ::supl::integer_sequence<                                                                             \
        decltype(std::declval<lhs>() OP std::declval<rhs>()),                                                  \
        (lhs_seq OP rhs_seq)...>                                                                               \
    {                                                                                                          \
        static_assert(                                                                                         \
          sizeof...(lhs_seq) == sizeof...(rhs_seq),                                                            \
          "Cannot perform operation on integer sequences of unequal "                                          \
          "length"                                                                                             \
        );                                                                                                     \
        return {};                                                                                             \
    }
...
#define integer_sequence_unary_operation(OP)                              \
    template <typename T, T... seq>                                       \
    constexpr auto                                                        \
    operator OP(::supl::integer_sequence<T, seq...>) noexcept             \
      -> ::supl::integer_sequence<                                        \
        decltype(OP std::declval<T>()), (OP seq)...>                      \
    {                                                                     \
        return {};                                                        \
    }
