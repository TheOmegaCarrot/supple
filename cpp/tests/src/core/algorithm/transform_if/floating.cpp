...
auto main() -> int
{
    ...
    const auto times_three {
      [](auto value) -> decltype(value)
      {
          return value * 3;
      }
    };
    ...
    supl::transform_if(
      test_double_input.begin(), test_double_input.end(),
      std::back_inserter(test_double_output), supl::greater_than(4.0),
      times_three
    );

    supl::for_each_both(
      test_double_output.begin(), test_double_output.end(),
      double_reference_output.begin(), double_reference_output.end(),
