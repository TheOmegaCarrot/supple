...
static_assert(std::is_same_v<
              std::variant<
                my_monostate, int, char, std::reference_wrapper<bool>>,
              tuple_element_variant_t<std::tuple<int, char, bool&>>>);
...
static_assert(std::is_same_v<
              std::variant<
                my_monostate, const int, char,
                std::reference_wrapper<const bool>>,
              tuple_element_variant_t<
                std::tuple<const int, char, const bool&>>>);
...
void test_variant(
  supl::test_results& results, const Variant& variant, const Value& value
)
{
    std::visit(
      [&results, &value](const auto& held)
      {
          if constexpr ( std::is_same_v<
                           supl::remove_cvref_t<decltype(held)>, Value> )
          {
...
void test_tuple_index(
  supl::test_results& results, Tuple&& tuple, const Expected& expected,
  std::size_t idx
)
{
    auto test_value {
